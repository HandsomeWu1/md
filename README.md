...
- **System 提示词可自定义**：设置弹窗底部「高级：System 提示词」展开后可整段编辑，它决定 AI 如何判断「对话」还是「改写文档」。改完可一键「恢复默认」；留空保存即回退默认。若删掉了 `%%REWRITE%%` 协议标记的说明，界面会提示「AI 将无法改写文档（只能对话）」——**只提示不阻止**，因为你可能就想要一个纯对话助手。
- **实现要点**：
